...
* `u` is also *stricter* than default ECMA-262, so a few legacy-but-tolerated spellings (`a{,3}`, a bare `}` or `]`, `\-`) are a syntax error on Scala.js while the JVM accepts them.
